...
export default function SongCard({ item, rank, onUpvote }) {
  ...
  const rankStyle =
    rankColors[rank] || "text-slate-600 border-white/8 bg-white/5";
  ...
  return (
    <div className="flex items-center gap-3 bg-muzix-surface border border-white/8 rounded-xl p-3 hover:border-white/15 transition-all duration-150 group">
      {/* Rank */}
      <div
        className={`flex-shrink-0 w-7 h-7 rounded-lg border text-xs font-black flex items-center justify-center ${rankStyle}`}
      >
        {rank}
      </div>
