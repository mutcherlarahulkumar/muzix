...
const roomTasks    = new Map(); // roomId → [{ id, text, done, createdBy, createdByName, ts }]
...
export function initSocket(io) {
  ...
  io.on('connection', (socket) => {
    ...
    // ── Spotlight sync (admin sets, all receive) ─────────────────────────────
    ...
    socket.on('pomodoro:control', ({ action, mode }) => {
      ...
      if (action === 'start' && !s.running) {
        s = { ...s, running: true, startedAt: Date.now() };
      } else if (action === 'pause' && s.running) {
        s = { ...s, running: false, elapsed: s.elapsed + (Date.now() - s.startedAt) / 1000, startedAt: null };
      } else if (action === 'reset') {
        s = { ...s, running: false, elapsed: 0, startedAt: null };
      } else if (action === 'skip') {
    ...
    socket.on('task:add', ({ text }) => {
      ...
      tasks.push({ id: `${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, text: text.trim().slice(0, 200), done: false, createdBy: currentUser.userId, createdByName: currentUser.username, ts: Date.now() });
